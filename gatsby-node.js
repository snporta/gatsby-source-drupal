...
exports.sourceNodes = async ({ boundActionCreators, getNode, hasNodeChanged, store, cache }, { baseUrl, apiBase }) => {
      ...
      if (datum.relationships) {
        _.each(datum.relationships, (v, k) => {
          ...
          if (_.isArray(v.data) && v.data.length > 0) {
            ...
            node.relationships[`${k}___NODE`] = _.compact(v.data.map(data => ids[data.id] ? data.id : null));
